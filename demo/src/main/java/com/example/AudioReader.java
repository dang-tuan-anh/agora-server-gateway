...
public class AudioReader {
    ...
    static BufferedInputStream file = null;

    private void sendOnePcmFrame(SampleOptions options, IAudioStreamer sender) {
        String fileName = options.audioFile;

        // Calculate byte size for 10ms audio samples
        int sampleSize = Short.BYTES * options.audio.numOfChannels; // sizeof(int16_t) in bytes is 2
        int samplesPer10ms = options.audio.sampleRate / 1000 * options.interval;
        ...
        try {
            ...
            // frameBuf
            sender.sendAudioPcmData(frameBuf, 0, samplesPer10ms, 2, options.audio.numOfChannels,
    ...
    public void sampleSendAudioTask(SampleOptions options, IAudioStreamer audioFrameSender, AtomicBoolean exitFlag) {
    ...
    public void readAudioUsingJavaCV(SampleOptions options, IAudioStreamer audioFrameSender, AtomicBoolean exitFlag) {
        ...
        int sampleSize = Short.BYTES  * options.audio.numOfChannels; // sizeof(int16_t) in bytes is 2
        int sampleRate = options.audio.sampleRate;
        ...
        try {
            ...
            while (true) {
                ...
                // Send audio frame
                audioFrameSender.sendAudioPcmData(dataBytes, 0, 640, sampleSize, options.audio.numOfChannels, sampleRate);
