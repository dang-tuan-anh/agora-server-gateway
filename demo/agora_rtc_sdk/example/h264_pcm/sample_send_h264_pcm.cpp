...
#define DEFAULT_WATCH_DIRECTORY "test_data"
...
struct SampleOptions {
  ...
  std::string localIP;
  std::string watchDirectory = DEFAULT_WATCH_DIRECTORY;
  struct {
...
static void print_frame(uint8_t *buffer, int size)
{
	int i = 0;
...
static void sendOnePcmFrame(const SampleOptions& options,
  ...
  // print_frame(frameBuf, sendBytes);
}
...
static void watchDirectory(SampleOptions& options,
                    agora::agora_refptr<agora::rtc::IVideoEncodedImageSender> videoH264FrameSender,
                    bool& exitFlag) {
    ...
    int watchDescriptor = inotify_add_watch(inotifyFd, options.watchDirectory.c_str(), IN_CLOSE_WRITE);
    if (watchDescriptor == -1) {
        std::cerr << "Error adding watch for " << options.watchDirectory << std::endl;
        close(inotifyFd);
        return;
    }
    ...
    while (!exitFlag) {
        ...
        for (int i = 0; i < length;) {
            ...
            if (event->mask & IN_CLOSE_WRITE) {
                ...
                if (hasExtension(fileName, ".h264")) {
                    std::cout << "A new .h264 file " << event->name << " has been created." << std::endl;
                    options.videoFile = options.watchDirectory + "/" + event->name;
                    SampleSendVideoH264Task(options, videoH264FrameSender, exitFlag);
                }
            }
...
int main(int argc, char* argv[]) {
  ...
  optParser.add_long_opt("localIP", &options.localIP,
                         "Local IP");
  optParser.add_long_opt("watchDirectory", &options.watchDirectory,
                          "The directory to watch for new video files");
  ...
  std::thread sendVideoThread(watchDirectory, std::ref(options), videoFrameSender, std::ref(exitFlag));
